...
public class GiantMallornTreeFeature extends Feature<NoneFeatureConfiguration> {
	...
	public GiantMallornTreeFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("the_hobbit_mod", "giant_mallorn_tree"));
		if (template == null)
			return false;
